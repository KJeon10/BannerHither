...
https://github.com/user-attachments/assets/2eff8d98-449f-4782-b07f-2d89372067ae
...
その少しあとに、BannerHitherが一度だけ質問します。アップデートを自動的に確認してよいかどうかです。[アップデート](#アップデート)を参照してください。
...
- Dockアイコンのないメニューバー専用アプリ。開始/停止の切り替え、ログイン時に開く、*アップデートを確認…*と「について」パネル、マウスを動かす時間を確保する3秒遅延のテスト通知、バグ報告用の*診断情報をコピー*。
- ユニバーサルバイナリ（AppleシリコンとIntel）。UIは英語、韓国語、日本語、簡体中国語、ドイツ語、フランス語、スペイン語に対応。
- 必要な権限はアクセシビリティのみ。データ収集はなく、ネットワーク通信は任意のアップデート確認だけです（[プライバシー](#プライバシー)を参照）。

## アップデート

BannerHitherはアップデートを自分でインストールしません。メニューの*アップデートを確認…*は、現在のバージョンを[最新リリース](https://github.com/KJeon10/BannerHither/releases/latest)と比較してダウンロードを案内します。同意すると1日1回バックグラウンドでも確認し、新しいバージョンがあればダイアログを出さずに、メニュー項目とメニューバーアイコンの点だけで知らせます。*このバージョンをスキップ*を選ぶと、次のバージョンが出るまでそのリリースを表示しません。Homebrewでインストールした場合は`brew upgrade --cask bannerhither`で同じことができます。
...
**通知がなぜ別のモニタに表示されるのですか？**
macOSは通知バナーを常に主ディスプレイ（システム設定 › ディスプレイでメニューバーがある画面）にだけ表示します。マウスポインタやアクティブなウインドウがどこにあっても同じで、別の画面で作業していると通知は毎回違う画面に出てしまいます。BannerHitherはバナーを、いま実際に見ている画面へ移動します。

**2台目のモニタや外部ディスプレイで通知を受け取れますか？**
はい。*マウスポインタのある画面*か*アクティブなウインドウのある画面*を選ぶか、*特定のディスプレイ*として外部ディスプレイを指定してください。ほかの設定は何も変えずに、2台目のモニタにバナーが表示されます。

**システム設定 › ディスプレイでメニューバーを別の画面にドラッグすればよいのでは？**
それではその画面が主ディスプレイになり、Dock、新しいウインドウの既定位置、メインの操作スペースまで一緒に移動します。しかも一度決めたら固定で、画面を行き来しても付いてきません。BannerHitherはディスプレイの配置には触れず、バナーだけを移動します。

**通知そのものが変わりますか？**
いいえ。NotificationCenterのウインドウごと移動するので、クリック、スワイプ、通知のアクションはそのまま機能し、通知の内容は読み取りません。時計をクリックして開く通知センターのパネルにも触れません。

**対応しているmacOSのバージョンは？**
macOS 14 Sonoma以降です。macOS 26 Tahoeで検証済みです（上の検証済みバージョンの表を参照）。

## 詳細設定

メニューの項目はすべて`io.github.kjeon10.BannerHither`の`UserDefaults`に保存されます。メニューにないオプションが3つあります。
...
| `updateFeedURL` | 未設定 | GitHubの`releases/latest`と同じJSON形式の代替リリースフィード。ローカルファイルを指定してアップデート確認をテストできます。起動時に一度だけ読み込みます。 |
...
BannerHitherは通知の内容を読みません。ネットワーク通信はアップデート確認だけです。同意した場合は1日1回、また*アップデートを確認…*を選んだときに、`api.github.com`から最新リリースの番号を取得します。このリクエストに含まれるのはアプリ名とバージョンだけで、他のHTTPS接続と同様にGitHubにはIPアドレスが伝わります。自動確認は許可するまでオフのままで、メニューからいつでも再びオフにできます。必要なら[ソースから自分でビルド](#ソースからビルド)することもできます。
