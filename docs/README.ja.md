...
順不同です。各項目は専用のブランチで開発し、動作を確認してから`main`にマージします。

- **注目している画面（Attention）** — *マウスポインタのある画面*も*アクティブなウインドウのある画面*も、ユーザーがどこを見ているかの推測にすぎず、日常的な場面でよく外れます。マウスを別の画面に置いたまま他の画面で入力しているときや、バナーが出た瞬間にマウスがまだ次の画面へ移動している途中のときなどです。その瞬間に実際に注目しているディスプレイを判断し、そこにバナーを置く表示位置オプションを追加します。
- **表示位置のルール** — 「Slackは左のモニタ、カレンダーは見ている画面、残りは一番遠い画面へ」のように一文で書くか、条件ピッカーで同じルールを組み立てて、バナーの表示先を指定します。
- **ポインタバッジ** — マウスポインタやテキストカーソルの横に小さなバッジを出して通知の到着を知らせ、その場で開けるようにします。作業の邪魔をせずに役立つものにする方法は検討中です。
