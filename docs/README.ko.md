...
잠시 뒤 BannerHither가 한 번만 묻습니다. 업데이트를 자동으로 확인해도 되는지입니다.
[업데이트](#업데이트) 절을 참고하세요.
...
- Dock 아이콘 없는 메뉴 막대 앱, 시작/중지 토글, 로그인 시 실행, *업데이트 확인…*과 정보 패널, 마우스를
  옮길 시간을 주는 3초 지연 테스트 알림, 버그 리포트용 *진단 정보 복사*.
- 유니버설 바이너리(Apple Silicon·Intel), 한국어·영어·일본어·중국어(간체)·독일어·프랑스어·스페인어 UI.
- 필요한 권한은 손쉬운 사용 하나뿐. 데이터 수집이 없으며, 네트워크 요청은 선택 사항인 업데이트 확인
  하나뿐입니다([개인정보](#개인정보) 참고).

## 로드맵

순서는 없습니다. 각 항목은 별도 브랜치에서 개발하고 동작이 확인되면 `main`에 병합합니다.

- **집중 중인 화면(Attention)** — *마우스 포인터가 있는 화면*과 *활성 창이 있는 화면*은 둘 다 사용자가
  어디를 보고 있는지에 대한 추측이고, 일상적인 상황에서 자주 틀립니다. 마우스는 한 화면에 둔 채 다른
  화면에서 타이핑하고 있거나, 배너가 뜨는 순간 마우스가 아직 다음 화면으로 건너가는 중일 때가 그렇습니다.
  그 순간 실제로 집중하고 있는 디스플레이를 알아내어 거기에 배너를 두는 표시 위치 옵션을 추가합니다.
- **배너 위치 규칙** — "슬랙은 왼쪽 모니터, 캘린더는 내가 보고 있는 화면, 나머지는 제일 먼 화면으로"처럼
  한 문장으로 적거나, 조건 선택기로 같은 규칙을 만들어 배너 위치를 지정합니다.
- **포인터 배지** — 마우스 포인터나 텍스트 커서 옆에 뜨는 작은 배지로 알림이 왔음을 알리고 그 자리에서
  바로 열 수 있게 합니다. 하던 작업을 방해하지 않으면서 쓸모 있게 만드는 방법은 아직 고민 중입니다.

## 업데이트

BannerHither는 업데이트를 직접 설치하지 않습니다. 메뉴의 *업데이트 확인…*은 현재 버전을
[최신 릴리스](https://github.com/KJeon10/BannerHither/releases/latest)와 비교해 다운로드를 안내합니다. 동의하면
하루에 한 번 백그라운드에서도 확인하며, 새 버전이 있으면 대화상자 없이 메뉴 항목과 메뉴 막대 아이콘의 점으로만
알립니다. *이 버전 건너뛰기*를 누르면 다음 버전이 나올 때까지 그 릴리스를 숨깁니다. Homebrew로 설치했다면
`brew upgrade --cask bannerhither`로 같은 일을 할 수 있습니다.
...
| `updateFeedURL` | 없음 | GitHub `releases/latest`와 같은 JSON 형태의 대체 릴리스 피드. 로컬 파일을 지정해 업데이트 확인을 테스트할 수 있습니다. 실행 시 한 번 읽습니다. |
...
BannerHither는 알림 내용을 읽지 않습니다. 네트워크 요청은 업데이트 확인 하나뿐입니다. 동의한 경우 하루에 한 번,
그리고 *업데이트 확인…*을 누를 때 `api.github.com`에서 최신 릴리스 번호를 가져옵니다. 이 요청은 앱 이름과 버전만
담으며, 다른 HTTPS 연결과 마찬가지로 GitHub에 IP 주소는 드러납니다. 자동 확인은 허용하기 전까지 꺼져 있고 메뉴에서
언제든 다시 끌 수 있습니다. 원한다면 [소스에서 직접 빌드](#소스에서-빌드)할 수도 있습니다.
